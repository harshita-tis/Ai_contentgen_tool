...
def _fetch_by_product_id(product_id: str, _domain: str = '', _token: str = '', _api_version: str = '') -> dict:
    ...
    if not domain or not token:
        ...
        if not shop:
            ...
            logger.error(f"[fetch_by_product_id] FAILED: No active shop configured in database for product_id={product_id}")
    ...
    if not product:
        ...
        logger.warning(f"[fetch_by_product_id] No product found for {gid} on shop '{domain}' — likely invalid ID or numeric handle")

        raise ValueError(f"Product ID '{product_id}' not found on shop '{domain}'.")

    logger.info(f"[fetch_by_product_id] Successfully fetched product ID {product_id} ('{product.get('title')}')")
...
def _fetch_by_storefront_url(raw_url: str) -> dict:
    ...
    if not resp.ok:
        ...
        logger.error(f"[fetch_by_storefront_url] HTTP {resp.status_code} body={resp.text[:500]}")
    ...
    try:
        ...
        logger.error(f"[fetch_by_storefront_url] Failed to parse storefront JSON for {url}: {parse_err}")
...
def _fetch_by_handle_admin(handle: str, _domain: str = '', _token: str = '', _api_version: str = '') -> dict:
    ...
    if not domain or not token:
        ...
        if not shop:
            ...
            logger.error(f"[fetch_by_handle_admin] FAILED: No active shop config found for handle '{handle}'")
    ...
    if not product:
        ...
        logger.warning(f"[fetch_by_handle_admin] No product found for handle '{handle}' on shop '{domain}'")

        raise ValueError(f"No product found for handle '{handle}' on shop '{domain}'.")

    logger.info(f"[fetch_by_handle_admin] Successfully fetched handle '{handle}' ('{product.get('title')}')")
...
def _fetch_shopify_product(raw_url: str, _domain: str = '', _token: str = '', _api_version: str = '') -> dict:
    ...
    if product_id:
        ...
        logger.info(f"[fetch_shopify_product] Extracted admin product_id='{product_id}'")

        try:

            return _fetch_by_product_id(product_id, _domain=_domain, _token=_token, _api_version=_api_version)
    ...
    if not domain or not token:
        ...
        if active_shop:
            ...
            logger.warning("[fetch_shopify_product] No active shop configured in database")
    ...
    if seg and seg.isdigit():
        ...
        try:
            ...
            logger.info(f"[fetch_shopify_product] Segment '{seg}' is numeric — trying internal product ID lookup")
    ...
    if seg and domain and token:
        ...
        try:
            ...
            logger.info(f"[fetch_shopify_product] Trying admin GraphQL handle lookup for handle='{seg}' on domain='{domain}'")
    ...
    if _is_storefront_url(raw_url):
        ...
        try:
            ...
            logger.info(f"[fetch_shopify_product] Trying storefront REST JSON lookup for '{raw_url}'")
    ...
    err_summary = "; ".join(attempt_errors) if attempt_errors else "No valid fetch mechanism succeeded"
...
def login_page():
    ...
    if request.method == 'POST':
        ...
        if user and user['password'] == password:
            ...
            if not next_target or next_target in ('/', '/login', f"{prefix}/login", f"{prefix}/login/"):
